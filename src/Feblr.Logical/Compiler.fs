...
module rec Grammar =
    ...
    type Term =
        ...
        | List of Term seq (* [...] *)
        | CompoundTerm of string * Term seq (* functor(t1, t2) := functor2, X, `hello`, 203430 *)
    ...
    let rec parseList (terms: Term seq) (tokens: Token seq) =
        match parseTerm tokens with
        | Ok (term, newTokens) ->
            let newTerms = Seq.append terms [ term ]
            let newTail = trim newTokens
                ...
                | None ->
                    Error
                        { token = None
                          message = "list should end with ]" }
            | Operator operator ->
                Ok(Oper operator,  Seq.tail tokens)
            | _ ->
                Error
    ...
    let rec parse (terms: Term seq) (tokens: Token seq) =
        if Seq.isEmpty tokens then
            ...
            | Identifier _ ->
                match parseTerm tokens with
                | Ok (term, tokens) -> parse (Seq.append terms [ term ]) (Seq.tail tokens |> trim)
                | Error err -> Error err
            | Whitespace
...
module Compiler =
    ...
    type Clause = { head: Head; body: Term seq }

    type CompilerError =
        { term: Term option
          message: string }

    let compile (clauses: Clause seq) (terms: Term seq) =
        if Seq.isEmpty terms then
            ...
            | CompoundTerm _ ->
                ...
                match Seq.tryHead tail with
                | Some nextTerm ->
                    Error { term = Some term; message = "" }
                | None ->
                    Error { term = None; message  = "fact or rule should end with period ." }
            | Oper _
            | Str _
            | Num _
            | List _
            | Var _ ->
                Error { term = Some term; message = "fact or rule should start with atom or compound term" }
